...
import { increaseCount, decreaseCount, removeFromCart, setCartOpen } from '../../state/cartSlice'
...
const CartMenu = () => {
    ...
    return (<Box display={isOpen ? 'block' : 'none'} backgroundColor="rgba(0,0,0,0.4)" position="fixed" zIndex={10} width='100%' left='0' top='0' overflow='auto'>
                ...
                <Box>
                    {cart.map((item) => {return(
                        <Box key={`${item.attributes.name}-${item.id}`}>
                            <FlexBox p='15px 0px'>
                                <Box flex="1 1 40%">
                                    <img src={`${import.meta.env.VITE_BACKENDURL}${item.attributes?.image?.data?.attributes?.formats?.medium.url}`} alt={item?.name} width='123px' height='164px' />
                                </Box>
                                <Box flex="1 1 60%">
                                    <FlexBox mb='5px'>
